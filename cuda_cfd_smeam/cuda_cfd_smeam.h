...
#ifndef cuda_cfd_smeam_h
#define cuda_cfd_smeam_h
...
class __declspec(dllexport)  CUDA_CFD_sMEAM
{
private:
#ifdef CFD_SMEAM_TIMINGS
	Timer *timer_;
	...
	CudaSpline *phi_spline_, *rho_spline_, *U_spline_, *f_spline_, *g_spline_;
	double cutoff_;
	...
	void Convert2DTo1DArray(meam_bond**,meam_bond*);
	void Convert2DTo1DArray(vec3d**, vec3d*);
	void Convert2DTo1DArray(int**, int*);
	void Convert1DTo2DArray(meam_bond*,meam_bond**);
...
public:
	CUDA_CFD_sMEAM(int max_number_of_central_forces);
	~CUDA_CFD_sMEAM();
...
inline void CUDA_CFD_sMEAM::computeCosThetaJIK(double r_ij_dir[3], double r_ik_dir[3], double &cos_theta_jik)
{
	cos_theta_jik = r_ij_dir[0] * r_ik_dir[0] + r_ij_dir[1] * r_ik_dir[1] + r_ij_dir[2] * r_ik_dir[2];
}


inline bool CUDA_CFD_sMEAM::compute_Theta4_kji(int atom_i, int number_of_bonds_j,
	meam_bond *bonds_list_i, meam_bond *bonds_list_j,
	double &T4kji)
{
